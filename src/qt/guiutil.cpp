...
#include <qt/gamcoinaddressvalidator.h>
...
void setupAddressWidget(QValidatedLineEdit *widget, QWidget *parent)
{
    ...
    widget->setPlaceholderText(QObject::tr("Enter a Gamcoin address (e.g. %1)").arg(
        QString::fromStdString(DummyAddress(Params()))));
    widget->setValidator(new GamcoinAddressEntryValidator(parent));
    widget->setCheckValidator(new GamcoinAddressCheckValidator(parent));
}
...
bool parseGamcoinURI(const QUrl &uri, SendCoinsRecipient *out)
{
    // return if URI is not valid or is no gamcoin: URI
    if(!uri.isValid() || uri.scheme() != QString("gamcoin"))
        return false;
    ...
    for (QList<QPair<QString, QString> >::iterator i = items.begin(); i != items.end(); i++)
    {
        ...
        else if (i->first == "amount")
        {
            if(!i->second.isEmpty())
            {
                if (!GamcoinUnits::parse(GamcoinUnit::GAM, i->second, &rv.amount)) {
                    return false;
                }
            }
            fShouldReturnFalse = false;
...
bool parseGamcoinURI(QString uri, SendCoinsRecipient *out)
{
    QUrl uriInstance(uri);
    return parseGamcoinURI(uriInstance, out);
}

QString formatGamcoinURI(const SendCoinsRecipient &info)
{
    bool bech_32 = info.address.startsWith(QString::fromStdString(Params().Bech32HRP() + "1"));

    QString ret = QString("gamcoin:%1").arg(bech_32 ? info.address.toUpper() : info.address);
    int paramCount = 0;

    if (info.amount)
    {
        ret += QString("?amount=%1").arg(GamcoinUnits::format(GamcoinUnit::GAM, info.amount, false, GamcoinUnits::SeparatorStyle::NEVER));
        paramCount++;
    }
...
bool openGamcoinConf()
{
    ...
    /* Open gamcoin.conf with the associated application */
...
fs::path static StartupShortcutPath()
{
    ...
    if (chain == ChainType::TESTNET) // Remove this special case when testnet CBaseChainParams::DataDir() is incremented to "testnet4"
        return GetSpecialFolderPath(CSIDL_STARTUP) / "Gamcoin (testnet).lnk";
    return GetSpecialFolderPath(CSIDL_STARTUP) / fs::u8path(strprintf("Gamcoin (%s).lnk", ChainTypeToString(chain)));
}
...
fs::path static GetAutostartFilePath()
{
    ...
    return GetAutostartDir() / fs::u8path(strprintf("gamcoin-%s.desktop", ChainTypeToString(chain)));
}
...
bool SetStartOnSystemStartup(bool fAutoStart)
{
    ...
    else
    {
        ...
        if (chain == ChainType::MAIN)
            ...
            optionFile << strprintf("Name=Gamcoin (%s)\n", ChainTypeToString(chain));
