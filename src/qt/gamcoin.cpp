...
static void RegisterMetaTypes()
{
    ...
    qRegisterMetaTypeStreamOperators<GamcoinUnit>("GamcoinUnit");
#else
    qRegisterMetaType<GamcoinUnit>("GamcoinUnit");
#endif
}
...
static void initTranslations(QTranslator &qtTranslatorBase, QTranslator &qtTranslator, QTranslator &translatorBase, QTranslator &translator)
{
    ...
    // Load e.g. gamcoin_de.qm (shortcut "de" needs to be defined in gamcoin.qrc)
    if (translatorBase.load(lang, ":/translations/")) {
        QApplication::installTranslator(&translatorBase);
    }

    // Load e.g. gamcoin_de_DE.qm (shortcut "de_DE" needs to be defined in gamcoin.qrc)
...
static const char* qt_argv = "gamcoin-qt";

GamcoinApplication::GamcoinApplication()
...
void GamcoinApplication::setupPlatformStyle()
{
    // UI per-platform customization
    // This must be done inside the GamcoinApplication constructor, or after it, because
    // PlatformStyle::instantiate requires a QApplication
    std::string platformName;
    platformName = gArgs.GetArg("-uiplatform", GamcoinGUI::DEFAULT_UIPLATFORM);
...
#ifdef ENABLE_WALLET
void GamcoinApplication::createPaymentServer()
{
    paymentServer = new PaymentServer(this);
}
#endif

bool GamcoinApplication::createOptionsModel(bool resetSettings)
{
...
void GamcoinApplication::createWindow(const NetworkStyle *networkStyle)
{
    window = new GamcoinGUI(node(), platformStyle, networkStyle, nullptr);
    connect(window, &GamcoinGUI::quitRequested, this, &GamcoinApplication::requestShutdown);
...
void GamcoinApplication::createSplashScreen(const NetworkStyle *networkStyle)
{
    assert(!m_splash);
...
void GamcoinApplication::createNode(interfaces::Init& init)
{
    assert(!m_node);
...
bool GamcoinApplication::baseInitialize()
{
    return node().baseInitialize();
}

void GamcoinApplication::startThread()
{
    assert(!m_executor);
    m_executor.emplace(node());

    /*  communication to and from thread */
    connect(&m_executor.value(), &InitExecutor::initializeResult, this, &GamcoinApplication::initializeResult);
    connect(&m_executor.value(), &InitExecutor::shutdownResult, this, [] {
        QCoreApplication::exit(0);
    });
    connect(&m_executor.value(), &InitExecutor::runawayException, this, &GamcoinApplication::handleRunawayException);
    connect(this, &GamcoinApplication::requestedInitialize, &m_executor.value(), &InitExecutor::initialize);
    connect(this, &GamcoinApplication::requestedShutdown, &m_executor.value(), &InitExecutor::shutdown);
}

void GamcoinApplication::parameterSetup()
{
...
void GamcoinApplication::InitPruneSetting(int64_t prune_MiB)
{
    optionsModel->SetPruneTargetGB(PruneMiBtoGB(prune_MiB));
}

void GamcoinApplication::requestInitialize()
{
...
void GamcoinApplication::requestShutdown()
{
...
void GamcoinApplication::initializeResult(bool success, interfaces::BlockAndHeaderTipInfo tip_info)
{
    ...
    if (success) {
        ...
        // gamcoin: URIs or payment requests:
        if (paymentServer) {
            connect(paymentServer, &PaymentServer::receivedPaymentRequest, window, &GamcoinGUI::handlePaymentRequest);
            connect(window, &GamcoinGUI::receivedURI, paymentServer, &PaymentServer::handleURIOrFile);
...
void GamcoinApplication::handleRunawayException(const QString &message)
{
...
void GamcoinApplication::handleNonFatalException(const QString& message)
{
...
WId GamcoinApplication::getMainWinId() const
{
    if (!window)
        return 0;
...
bool GamcoinApplication::event(QEvent* e)
{
...
static void SetupUIArgs(ArgsManager& argsman)
{
    ...
    argsman.AddArg("-uiplatform", strprintf("Select platform to customize UI for (one of windows, macosx, other; default: %s)", GamcoinGUI::DEFAULT_UIPLATFORM), ArgsManager::ALLOW_ANY | ArgsManager::DEBUG_ONLY, OptionsCategory::GUI);
}
...
int GuiMain(int argc, char* argv[])
{
    ...
    Q_INIT_RESOURCE(gamcoin);
    Q_INIT_RESOURCE(gamcoin_locale);
    ...
    for (int i = 1; i < argc; i++) {
...
#ifdef ENABLE_WALLET
        if (arg.startsWith(GAMCOIN_IPC_PREFIX, Qt::CaseInsensitive)) {
            invalid_token &= false;
        ...
        if (invalid_token) {
            InitError(Untranslated(strprintf("Command line contains unexpected token '%s', see gamcoin-qt -h for a list of options.", argv[i])));
            QMessageBox::critical(nullptr, PACKAGE_NAME,
                                  // message cannot be translated because translations have not been initialized
                                  QString::fromStdString("Command line contains unexpected token '%1', see gamcoin-qt -h for a list of options.").arg(QString::fromStdString(argv[i])));
            return EXIT_FAILURE;
        }
    }
    ...
    /// 6-7. Parse gamcoin.conf, determine network, switch to network specific
    ...
    // gamcoin: links repeatedly have their payment requests routed to this process:
    if (WalletModel::isWalletEnabled()) {
