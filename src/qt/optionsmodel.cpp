...
/** Call node.updateRwSetting() with Gamcoin 22.x workaround. */
static void UpdateRwSetting(interfaces::Node& node, OptionsModel::OptionID option, const std::string& suffix, const common::SettingsValue& value)
{
    if (value.isNum() &&
        ...
        // because Gamcoin 22.x releases try to read these specific settings as
        ...
        // in later releases by https://github.com/gamcoin/gamcoin/pull/24498.
        // If new numeric settings are added, they can be written as numbers
        // instead of strings, because gamcoin 22.x will not try to read these.
...
//! Convert enabled/size values to gamcoin -prune setting.
...
//! Get pruning enabled value to show in GUI from gamcoin -prune setting.
...
//! Get pruning size value to show in GUI from gamcoin -prune setting. If
...
bool OptionsModel::Init(bilingual_str& error)
{
    ...
    // Display
    if (!settings.contains("DisplayGamcoinUnit")) {
        settings.setValue("DisplayGamcoinUnit", QVariant::fromValue(GamcoinUnit::GAM));
    }
    QVariant unit = settings.value("DisplayGamcoinUnit");
    if (unit.canConvert<GamcoinUnit>()) {
        m_display_gamcoin_unit = unit.value<GamcoinUnit>();
    } else {
        m_display_gamcoin_unit = GamcoinUnit::GAM;
        settings.setValue("DisplayGamcoinUnit", QVariant::fromValue(m_display_gamcoin_unit));
    }
...
void OptionsModel::SetPruneTargetGB(int prune_target_gb)
{
    ...
    // from saved value. Avoid writing settings.json if gamcoin.conf value
...
#endif
    case DisplayUnit:
        return QVariant::fromValue(m_display_gamcoin_unit);
...
{
    if (new_unit.isNull() || new_unit.value<GamcoinUnit>() == m_display_gamcoin_unit) return;
    m_display_gamcoin_unit = new_unit.value<GamcoinUnit>();
    QSettings settings;
    settings.setValue("DisplayGamcoinUnit", QVariant::fromValue(m_display_gamcoin_unit));
    Q_EMIT displayUnitChanged(m_display_gamcoin_unit);
}
...
void OptionsModel::checkAndMigrate()
{
    ...
    if (settingsVersion < CLIENT_VERSION)
    {
        ...
        // see https://github.com/gamcoin/gamcoin/pull/8273
    ...
    // (https://github.com/gamcoin-core/gui/issues/567).
