...
const std::string GamcoinGUI::DEFAULT_UIPLATFORM =
...
GamcoinGUI::GamcoinGUI(interfaces::Node& node, const PlatformStyle *_platformStyle, const NetworkStyle *networkStyle, QWidget *parent) :
    QMainWindow(parent),
    m_node(node),
    ...
    m_network_style(networkStyle)
{
    ...
    if(enableWallet)
    {
        ...
        connect(walletFrame, &WalletFrame::createWalletButtonClicked, this, &GamcoinGUI::createWallet);
    ...
    connect(labelBlocksIcon, &GUIUtil::ClickableLabel::clicked, this, &GamcoinGUI::showModalOverlay);
    connect(progressBar, &GUIUtil::ClickableProgressBar::clicked, this, &GamcoinGUI::showModalOverlay);
...
void GamcoinGUI::createActions()
{
    ...
    sendCoinsAction->setStatusTip(tr("Send coins to a Gamcoin address"));
    ...
    receiveCoinsAction->setStatusTip(tr("Request payments (generates QR codes and gamcoin: URIs)"));
    ...
    connect(overviewAction, &QAction::triggered, this, &GamcoinGUI::gotoOverviewPage);
    connect(sendCoinsAction, &QAction::triggered, [this]{ showNormalIfMinimized(); });
    connect(sendCoinsAction, &QAction::triggered, [this]{ gotoSendCoinsPage(); });
    connect(receiveCoinsAction, &QAction::triggered, [this]{ showNormalIfMinimized(); });
    connect(receiveCoinsAction, &QAction::triggered, this, &GamcoinGUI::gotoReceiveCoinsPage);
    connect(historyAction, &QAction::triggered, [this]{ showNormalIfMinimized(); });
    connect(historyAction, &QAction::triggered, this, &GamcoinGUI::gotoHistoryPage);
#endif // ENABLE_WALLET
    ...
    signMessageAction->setStatusTip(tr("Sign messages with your Gamcoin addresses to prove you own them"));
    verifyMessageAction = new QAction(tr("&Verify message…"), this);
    verifyMessageAction->setStatusTip(tr("Verify messages to ensure they were signed with specified Gamcoin addresses"));
    m_load_psbt_action = new QAction(tr("&Load PSBT from file…"), this);
    m_load_psbt_action->setStatusTip(tr("Load Partially Signed Gamcoin Transaction"));
    m_load_psbt_clipboard_action = new QAction(tr("Load PSBT from &clipboard…"), this);
    m_load_psbt_clipboard_action->setStatusTip(tr("Load Partially Signed Gamcoin Transaction from clipboard"));
    ...
    openAction->setStatusTip(tr("Open a gamcoin: URI"));
    ...
    showHelpMessageAction->setStatusTip(tr("Show the %1 help message to get a list with possible Gamcoin command-line options").arg(PACKAGE_NAME));
    ...
    connect(quitAction, &QAction::triggered, this, &GamcoinGUI::quitRequested);
    connect(aboutAction, &QAction::triggered, this, &GamcoinGUI::aboutClicked);
    connect(aboutQtAction, &QAction::triggered, qApp, QApplication::aboutQt);
    connect(optionsAction, &QAction::triggered, this, &GamcoinGUI::optionsClicked);
    connect(showHelpMessageAction, &QAction::triggered, this, &GamcoinGUI::showHelpMessageClicked);
    connect(openRPCConsoleAction, &QAction::triggered, this, &GamcoinGUI::showDebugWindow);
    ...
    if(walletFrame)
    {
        ...
        connect(openAction, &QAction::triggered, this, &GamcoinGUI::openClicked);
        connect(m_open_wallet_menu, &QMenu::aboutToShow, [this] {
            ...
            for (const std::pair<const std::string, bool>& i : m_wallet_controller->listWalletDir()) {
                ...
                connect(action, &QAction::triggered, [this, path] {
                    ...
                    connect(activity, &OpenWalletActivity::opened, this, &GamcoinGUI::setCurrentWallet, Qt::QueuedConnection);
        ...
        connect(m_restore_wallet_action, &QAction::triggered, [this] {
            ...
            connect(activity, &RestoreWalletActivity::restored, this, &GamcoinGUI::setCurrentWallet, Qt::QueuedConnection);
        ...
        connect(m_create_wallet_action, &QAction::triggered, this, &GamcoinGUI::createWallet);
        connect(m_close_all_wallets_action, &QAction::triggered, [this] {
            m_wallet_controller->closeAllWallets(this);
        });
        connect(m_migrate_wallet_action, &QAction::triggered, [this] {
            auto activity = new MigrateWalletActivity(m_wallet_controller, this);
            connect(activity, &MigrateWalletActivity::migrated, this, &GamcoinGUI::setCurrentWallet);
            activity->migrate(walletFrame->currentWalletModel());
        });
        connect(m_mask_values_action, &QAction::toggled, this, &GamcoinGUI::setPrivacy);
        connect(m_mask_values_action, &QAction::toggled, this, &GamcoinGUI::enableHistoryAction);
    }
#endif // ENABLE_WALLET

    connect(new QShortcut(QKeySequence(Qt::CTRL | Qt::SHIFT | Qt::Key_C), this), &QShortcut::activated, this, &GamcoinGUI::showDebugWindowActivateConsole);
    connect(new QShortcut(QKeySequence(Qt::CTRL | Qt::SHIFT | Qt::Key_D), this), &QShortcut::activated, this, &GamcoinGUI::showDebugWindow);
}

void GamcoinGUI::createMenuBar()
{
    appMenuBar = menuBar();
...
void GamcoinGUI::createToolBars()
{
    if(walletFrame)
    {
        ...
        connect(m_wallet_selector, qOverload<int>(&QComboBox::currentIndexChanged), this, &GamcoinGUI::setCurrentWalletBySelectorIndex);
...
void GamcoinGUI::setClientModel(ClientModel *_clientModel, interfaces::BlockAndHeaderTipInfo* tip_info)
{
    ...
    if(_clientModel)
    {
        ...
        connect(_clientModel, &ClientModel::numConnectionsChanged, this, &GamcoinGUI::setNumConnections);
        connect(_clientModel, &ClientModel::networkActiveChanged, this, &GamcoinGUI::setNetworkActive);
        ...
        connect(_clientModel, &ClientModel::numBlocksChanged, this, &GamcoinGUI::setNumBlocks);
        ...
        connect(_clientModel, &ClientModel::showProgress, this, &GamcoinGUI::showProgress);
...
#ifdef ENABLE_WALLET
void GamcoinGUI::enableHistoryAction(bool privacy)
{
...
void GamcoinGUI::setWalletController(WalletController* wallet_controller, bool show_loading_minimized)
{
    ...
    GUIUtil::ExceptionSafeConnect(wallet_controller, &WalletController::walletAdded, this, &GamcoinGUI::addWallet);
    connect(wallet_controller, &WalletController::walletRemoved, this, &GamcoinGUI::removeWallet);
...
WalletController* GamcoinGUI::getWalletController()
{
    return m_wallet_controller;
}

void GamcoinGUI::addWallet(WalletModel* walletModel)
{
    ...
    connect(wallet_view, &WalletView::outOfSyncWarningClicked, this, &GamcoinGUI::showModalOverlay);
    connect(wallet_view, &WalletView::transactionClicked, this, &GamcoinGUI::gotoHistoryPage);
    connect(wallet_view, &WalletView::coinsSent, this, &GamcoinGUI::gotoHistoryPage);
    connect(wallet_view, &WalletView::message, [this](const QString& title, const QString& message, unsigned int style) {
        this->message(title, message, style);
    });
    connect(wallet_view, &WalletView::encryptionStatusChanged, this, &GamcoinGUI::updateWalletStatus);
    connect(wallet_view, &WalletView::incomingTransaction, this, &GamcoinGUI::incomingTransaction);
    connect(this, &GamcoinGUI::setPrivacy, wallet_view, &WalletView::setPrivacy);
...
void GamcoinGUI::removeWallet(WalletModel* walletModel)
{
...
void GamcoinGUI::setCurrentWallet(WalletModel* wallet_model)
{
    if (!walletFrame || !m_wallet_controller) return;
...
void GamcoinGUI::setCurrentWalletBySelectorIndex(int index)
{
    WalletModel* wallet_model = m_wallet_selector->itemData(index).value<WalletModel*>();
    if (wallet_model) setCurrentWallet(wallet_model);
}

void GamcoinGUI::removeAllWallets()
{
    if(!walletFrame)
        return;
    setWalletActionsEnabled(false);
...
void GamcoinGUI::setWalletActionsEnabled(bool enabled)
{
...
void GamcoinGUI::createTrayIcon()
{
...
void GamcoinGUI::createTrayIconMenu()
{
#ifndef Q_OS_MACOS
    if (!trayIcon) return;
#endif // Q_OS_MACOS
    ...
    show_hide_action = trayIconMenu->addAction(QString(), this, &GamcoinGUI::toggleHidden);
...
void GamcoinGUI::optionsClicked()
{
    openOptionsDialogWithTab(OptionsDialog::TAB_MAIN);
}

void GamcoinGUI::aboutClicked()
{
    if(!clientModel)
        return;
...
void GamcoinGUI::showDebugWindow()
{
    GUIUtil::bringToFront(rpcConsole);
    Q_EMIT consoleShown(rpcConsole);
}

void GamcoinGUI::showDebugWindowActivateConsole()
{
    rpcConsole->setTabFocus(RPCConsole::TabTypes::CONSOLE);
    showDebugWindow();
}

void GamcoinGUI::showHelpMessageClicked()
{
    GUIUtil::bringToFront(helpMessageDialog);
}

#ifdef ENABLE_WALLET
void GamcoinGUI::openClicked()
{
...
void GamcoinGUI::gotoOverviewPage()
{
    overviewAction->setChecked(true);
    if (walletFrame) walletFrame->gotoOverviewPage();
}

void GamcoinGUI::gotoHistoryPage()
{
    historyAction->setChecked(true);
    if (walletFrame) walletFrame->gotoHistoryPage();
}

void GamcoinGUI::gotoReceiveCoinsPage()
{
    receiveCoinsAction->setChecked(true);
    if (walletFrame) walletFrame->gotoReceiveCoinsPage();
}

void GamcoinGUI::gotoSendCoinsPage(QString addr)
{
    sendCoinsAction->setChecked(true);
    if (walletFrame) walletFrame->gotoSendCoinsPage(addr);
}

void GamcoinGUI::gotoSignMessageTab(QString addr)
{
    if (walletFrame) walletFrame->gotoSignMessageTab(addr);
}

void GamcoinGUI::gotoVerifyMessageTab(QString addr)
{
    if (walletFrame) walletFrame->gotoVerifyMessageTab(addr);
}
void GamcoinGUI::gotoLoadPSBT(bool from_clipboard)
{
    if (walletFrame) walletFrame->gotoLoadPSBT(from_clipboard);
}
#endif // ENABLE_WALLET

void GamcoinGUI::updateNetworkState()
{
    ...
    if (m_node.getNetworkActive()) {
        ...
        tooltip = tr("%n active connection(s) to Gamcoin network.", "", count);
    } else {
        //: A substring of the tooltip.
...
void GamcoinGUI::setNumConnections(int count)
{
    updateNetworkState();
}

void GamcoinGUI::setNetworkActive(bool network_active)
{
    updateNetworkState();
...
void GamcoinGUI::updateHeadersSyncProgressLabel()
{
...
void GamcoinGUI::updateHeadersPresyncProgressLabel(int64_t height, const QDateTime& blockDate)
{
...
void GamcoinGUI::openOptionsDialogWithTab(OptionsDialog::Tab tab)
{
    if (!clientModel || !clientModel->getOptionsModel())
        return;

    auto dlg = new OptionsDialog(this, enableWallet);
    connect(dlg, &OptionsDialog::quitOnReset, this, &GamcoinGUI::quitRequested);
...
void GamcoinGUI::setNumBlocks(int count, const QDateTime& blockDate, double nVerificationProgress, SyncType synctype, SynchronizationState sync_state)
{
...
void GamcoinGUI::createWallet()
{
    ...
    connect(activity, &CreateWalletActivity::created, this, &GamcoinGUI::setCurrentWallet);
    connect(activity, &CreateWalletActivity::created, rpcConsole, &RPCConsole::setCurrentWallet);
    activity->create();
#endif // ENABLE_WALLET
}

void GamcoinGUI::message(const QString& title, QString message, unsigned int style, bool* ret, const QString& detailed_message)
{
...
void GamcoinGUI::changeEvent(QEvent *e)
{
    ...
    if(e->type() == QEvent::WindowStateChange)
    {
        if(clientModel && clientModel->getOptionsModel() && clientModel->getOptionsModel()->getMinimizeToTray())
        {
            ...
            {
                QTimer::singleShot(0, this, &GamcoinGUI::hide);
                e->ignore();
            }
            else if((wsevt->oldState() & Qt::WindowMinimized) && !isMinimized())
            {
                QTimer::singleShot(0, this, &GamcoinGUI::show);
                e->ignore();
            }
        }
    }
...
void GamcoinGUI::closeEvent(QCloseEvent *event)
{
#ifndef Q_OS_MACOS // Ignored on Mac
    if(clientModel && clientModel->getOptionsModel())
    {
...
void GamcoinGUI::showEvent(QShowEvent *event)
{
...
#ifdef ENABLE_WALLET
void GamcoinGUI::incomingTransaction(const QString& date, GamcoinUnit unit, const CAmount& amount, const QString& type, const QString& address, const QString& label, const QString& walletName)
{
    // On new transaction, make an info balloon
    QString msg = tr("Date: %1\n").arg(date) +
                  tr("Amount: %1\n").arg(GamcoinUnits::formatWithUnit(unit, amount, true));
...
void GamcoinGUI::dragEnterEvent(QDragEnterEvent *event)
{
    // Accept only URIs
...
void GamcoinGUI::dropEvent(QDropEvent *event)
{
    if(event->mimeData()->hasUrls())
    {
...
bool GamcoinGUI::eventFilter(QObject *object, QEvent *event)
{
...
#ifdef ENABLE_WALLET
bool GamcoinGUI::handlePaymentRequest(const SendCoinsRecipient& recipient)
{
    // URI has to be valid
...
void GamcoinGUI::setHDStatus(bool privkeyDisabled, int hdEnabled)
{
...
void GamcoinGUI::setEncryptionStatus(int status)
{
    switch(status)
    {
...
void GamcoinGUI::updateWalletStatus()
{
    assert(walletFrame);
...
void GamcoinGUI::updateProxyIcon()
{
    std::string ip_port;
...
void GamcoinGUI::updateWindowTitle()
{
...
void GamcoinGUI::showNormalIfMinimized(bool fToggleHidden)
{
    if(!clientModel)
        return;
...
void GamcoinGUI::toggleHidden()
{
    showNormalIfMinimized(true);
}

void GamcoinGUI::detectShutdown()
{
...
void GamcoinGUI::showProgress(const QString &title, int nProgress)
{
    if (nProgress == 0) {
...
void GamcoinGUI::showModalOverlay()
{
    if (modalOverlay && (progressBar->isVisible() || modalOverlay->isLayerVisible()))
        modalOverlay->toggleVisibility();
}

static bool ThreadSafeMessageBox(GamcoinGUI* gui, const bilingual_str& message, const std::string& caption, unsigned int style)
{
    ...
    if (message.original != message.translated) {
        detailed_message = GamcoinGUI::tr("Original message:") + "\n" + QString::fromStdString(message.original);
    }
...
void GamcoinGUI::subscribeToCoreSignals()
{
...
void GamcoinGUI::unsubscribeFromCoreSignals()
{
...
bool GamcoinGUI::isPrivacyModeActivated() const
{
    ...
    : m_platform_style{platformStyle}
{
    ...
    QList<GamcoinUnit> units = GamcoinUnits::availableUnits();
    int max_width = 0;
    const QFontMetrics fm(font());
    for (const GamcoinUnit unit : units) {
        max_width = qMax(max_width, GUIUtil::TextWidth(fm, GamcoinUnits::longName(unit)));
    }
...
{
    menu = new QMenu(this);
    for (const GamcoinUnit u : GamcoinUnits::availableUnits()) {
        menu->addAction(GamcoinUnits::longName(u))->setData(QVariant::fromValue(u));
    }
...
void UnitDisplayStatusBarControl::updateDisplayUnit(GamcoinUnit newUnits)
{
    setText(GamcoinUnits::longName(newUnits));
}
