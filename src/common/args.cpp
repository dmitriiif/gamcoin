...
const char * const GAMCOIN_CONF_FILENAME = "gamcoin.conf";
const char * const GAMCOIN_SETTINGS_FILENAME = "settings.json";
...
bool ArgsManager::ParseParameters(int argc, const char* const argv[], std::string& error)
{
    ...
    for (int i = 1; i < argc; i++) {
        ...
        if (key == "-") break; //gamcoin-tx using stdin
...
{
    fs::path settings = GetPathArg("-settings", GAMCOIN_SETTINGS_FILENAME);
    if (settings.empty()) {
        return false;
    }
    if (backup) {
...
{
    // Windows: C:\Users\Username\AppData\Roaming\Gamcoin
    // macOS: ~/Library/Application Support/Gamcoin
    // Unix-like: ~/.gamcoin
#ifdef WIN32
    // Windows
...
#ifdef MAC_OSX
    // macOS
    return pathRet / "Library/Application Support/Gamcoin";
#else
    // Unix-like
