...
class AvoidReuseTest(GamcoinTestFramework):
    ...
    def test_sending_from_reused_address_without_avoid_reuse(self):
        '''
        Test the same as test_sending_from_reused_address_fails, except send the 10 GAM with
        the avoid_reuse flag set to false. This means the 10 GAM send should succeed,
    ...
    def test_sending_from_reused_address_fails(self, second_addr_type):
        ...
        [0] sends 10 GAM to A.
        [1] spends 5 GAM from A. (leaving roughly 5 GAM useable)
        [0] sends 10 GAM to A again.
        [1] tries to spend 10 GAM (fails; dirty).
        [1] tries to spend 4 GAM (succeeds; change address sufficient)
        '''
    ...
    def test_full_destination_group_is_preferred(self):
        '''
        Test the case where [1] only has 101 outputs of 1 GAM in the same reused
        address and tries to send a small payment of 0.5 GAM. The wallet
        should use 100 outputs from the reused address as inputs and not a
        single 1 GAM input, in order to join several outputs from the reused
        address.
        '''
        ...
        # Send 101 outputs of 1 GAM to the same, reused address in the wallet
        for _ in range(101):
            self.nodes[0].sendtoaddress(new_addr, 1)
    ...
    def test_all_destination_groups_are_used(self):
        '''
        Test the case where [1] only has 202 outputs of 1 GAM in the same reused
        address and tries to send a payment of 200.5 GAM. The wallet
        ...
        # Send 202 outputs of 1 GAM to the same, reused address in the wallet
        for _ in range(202):
            self.nodes[0].sendtoaddress(new_addr, 1)
