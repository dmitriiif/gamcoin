...
`test/functional/test_framework/test_framework.py:GamcoinTestFramework.main`.
...
BASE_SCRIPTS = [
    ...
    'interface_gamcoin_cli.py --legacy-wallet',
    'interface_gamcoin_cli.py --descriptors',
...
def main():
    ...
    enable_gamcoind = config["components"].getboolean("ENABLE_GAMCOIND")

    if not enable_gamcoind:
...
def run_tests(*, test_list, src_dir, build_dir, tmpdir, jobs=1, enable_coverage=False, args=None, combined_logs_len=0, failfast=False, use_term_control, skipunit=False):
    ...
    # Warn if gamcoind is already running
    try:
        # pgrep exits with code zero when one or more matching processes found
        if subprocess.run(["pgrep", "-x", "gamcoind"], stdout=subprocess.DEVNULL).returncode == 0:
            print("%sWARNING!%s There is already a gamcoind process running on this system. Tests may fail unexpectedly due to resource contention!" % (BOLD[1], BOLD[0]))
    except OSError:
        # pgrep not supported
        pass
    ...
    # a hard link or a copy on any platform. See https://github.com/gamcoin/gamcoin/pull/27561.
...
class RPCCoverage():
    ...
    commands per `gamcoin-cli help` (`rpc_interface.txt`).
