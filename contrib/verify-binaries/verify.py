...
"""Script for verifying Gamcoin Core release binaries.

This script attempts to download the sum file SHA256SUMS and corresponding
signature file SHA256SUMS.asc from gamcoincore.org and gamcoin.org and
compares them.
...
The builder keys are available in the guix.sigs repo:
    ...
    https://github.com/gamcoin-core/guix.sigs/tree/main/builder-keys
...
HOST1 = "https://gamcoincore.org"
HOST2 = "https://gamcoin.org"
VERSIONPREFIX = "gamcoin-core-"
...
def verify_shasums_signature(
   ...
   ReturnCode, list[SigData], list[SigData], list[SigData], list[SigData]
    ...
    # which pubkeys within the Gamcoin community do we trust for the purposes of
    # binary verification?
    trusted_keys = set()
...
def verify_published_handler(args: argparse.Namespace) -> ReturnCode:
    WORKINGDIR = Path(tempfile.gettempdir()) / f"gamcoin_verify_binaries.{args.version}"
    ...
    # remove binaries that are known not to be hosted by gamcoincore.org
...
def main():
    ...
    pub_parser.add_argument(
        'version', type=str, help=(
            f'version of the gamcoin release to download; of the format '
    ...
    pub_parser.add_argument(
        ...
        help=(
            ...
            '(Sometimes gamcoin.org lags behind gamcoincore.org.)')
    )
