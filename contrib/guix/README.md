# Bootstrappable Gamcoin Core Builds

This directory contains the files necessary to perform bootstrappable Gamcoin
Core builds.
...
To build Gamcoin Core reproducibly with all default options, invoke the
...
2. The `guix-codesign` command ***requires*** a `DETACHED_SIGS_REPO` flag.
    * _**DETACHED_SIGS_REPO**_
      ...
      Gamcoin Core version being built.
...
```
env DETACHED_SIGS_REPO=<path/to/gamcoin-detached-sigs> ./contrib/guix/guix-codesign
```
...
repository](https://github.com/gamcoin-core/guix.sigs).
