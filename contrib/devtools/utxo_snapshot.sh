#!/usr/bin/env bash
#
# Copyright (c) 2019-2023 The Gamcoin Core developers
...
if (( $# < 3 )); then
  echo 'Usage: utxo_snapshot.sh <generate-at-height> <snapshot-out-path> <gamcoin-cli-call ...>'
  echo
  ...
  echo 'Examples:'
  echo
  echo "  ./contrib/devtools/utxo_snapshot.sh 570000 utxo.dat ./src/gamcoin-cli -datadir=\$(pwd)/testdata"
  echo '  ./contrib/devtools/utxo_snapshot.sh 570000 - ./src/gamcoin-cli'
  exit 1
fi
...
GAMCOIN_CLI_CALL="${*} -rpcclienttimeout=9999999"

# Check if the node is pruned and get the pruned block height
PRUNED=$( ${GAMCOIN_CLI_CALL} getblockchaininfo | awk '/pruneheight/ {print $2}' | tr -d ',' )
...
function cleanup {
  ...
  ${GAMCOIN_CLI_CALL} reconsiderblock "${PIVOT_BLOCKHASH}"

  if $NETWORK_DISABLED; then
    (>&2 echo "Restoring network activity")
    ${GAMCOIN_CLI_CALL} setnetworkactive true
  fi
}
...
if [[ "$REPLY" =~ ^[Yy]*$ || -z "$REPLY" ]]; then
  ...
  ${GAMCOIN_CLI_CALL} setnetworkactive false
else
  (>&2 echo "Network activity remains enabled")
fi

# Block we'll invalidate/reconsider to rewind/fast-forward the chain.
PIVOT_BLOCKHASH=$($GAMCOIN_CLI_CALL getblockhash $(( GENERATE_AT_HEIGHT + 1 )) )
...
${GAMCOIN_CLI_CALL} invalidateblock "${PIVOT_BLOCKHASH}"

if [[ "${OUTPUT_PATH}" = "-" ]]; then
  (>&2 echo "Generating txoutset info...")
  ${GAMCOIN_CLI_CALL} gettxoutsetinfo | grep hash_serialized_3 | sed 's/^.*: "\(.\+\)\+",/\1/g'
else
  (>&2 echo "Generating UTXO snapshot...")
  ${GAMCOIN_CLI_CALL} dumptxoutset "${OUTPUT_PATH}"
fi
