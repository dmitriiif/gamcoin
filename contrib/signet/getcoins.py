#!/usr/bin/env python3
# Copyright (c) 2020-2022 The Gamcoin Core developers
...
parser = argparse.ArgumentParser(description='Script to get coins from a faucet.', epilog='You may need to start with double-dash (--) when providing gamcoin-cli arguments.')
parser.add_argument('-c', '--cmd', dest='cmd', default='gamcoin-cli', help='gamcoin-cli command to use')
...
parser.add_argument('-a', '--addr', dest='addr', default='', help='Gamcoin address to which the faucet should send')
...
parser.add_argument('gamcoin_cli_args', nargs='*', help='Arguments to pass on to gamcoin-cli (default: -signet)')

args = parser.parse_args()

if args.gamcoin_cli_args == []:
    args.gamcoin_cli_args = ['-signet']


def gamcoin_cli(rpc_command_and_params):
    argv = [args.cmd] + args.gamcoin_cli_args + rpc_command_and_params
    try:
...
if args.faucet.lower() == DEFAULT_GLOBAL_FAUCET:
    ...
    curr_signet_hash = gamcoin_cli(['getblockhash', '1'])
...
if args.addr == '':
    ...
    args.addr = gamcoin_cli(['getnewaddress', 'faucet', 'bech32'])
