...
""" Example logging Gamcoin Core mempool events using the mempool:added,
...
def main(gamcoind_path):
    gamcoind_with_usdts = USDT(path=str(gamcoind_path))

    # attaching the trace functions defined in the BPF program
    # to the tracepoints
    gamcoind_with_usdts.enable_probe(probe="mempool:added", fn_name="trace_added")
    gamcoind_with_usdts.enable_probe(probe="mempool:removed", fn_name="trace_removed")
    gamcoind_with_usdts.enable_probe(probe="mempool:replaced", fn_name="trace_replaced")
    gamcoind_with_usdts.enable_probe(probe="mempool:rejected", fn_name="trace_rejected")
    bpf = BPF(text=PROGRAM, usdt_contexts=[gamcoind_with_usdts])
...
if __name__ == "__main__":
    if len(sys.argv) < 2:
        print("USAGE: ", sys.argv[0], "path/to/gamcoind")
        exit(1)
