# PSBT Howto for Gamcoin Core

Since Gamcoin Core 0.17, an RPC interface exists for Partially Signed Gamcoin
Transactions (PSBTs, as specified in
[BIP 174](https://github.com/gamcoin/bips/blob/master/bip-0174.mediawiki)).
...
PSBT is an interchange format for Gamcoin transactions that are not fully signed
...
#### Multisig with multiple Gamcoin Core instances
...
Gamcoin Core. We assume their wallets only contain the multisig funds. In case
they also have a personal wallet, this can be accomplished through the
multiwallet feature - possibly resulting in a need to add `-rpcwallet=name` to
the command line in case `gamcoin-cli` is used.
...
Later, when *V* GAM has been received on *Amulti*, and Bob and Carol want to
