# CJDNS support in Gamcoin Core

It is possible to run Gamcoin Core over CJDNS, an encrypted IPv6 network that
...
done outside of Gamcoin Core, similarly to a VPN (either in the host/OS or on
...
redundancy and robustness for both the Gamcoin network and individual nodes.
...
before it will work with your Gamcoin Core node. This is described in steps
...
## Run Gamcoin Core with CJDNS

Once you are connected to the CJDNS network, the following Gamcoin Core
...
When enabled, this option tells Gamcoin Core that it is running in an
...
IPv6 local network. This helps Gamcoin Core perform better address management:
...
CJDNS support was added to Gamcoin Core in version 23.0 and there may be fewer
CJDNS peers than Tor or IP ones. You can use `gamcoin-cli -addrinfo` to see the
...
[doc/tor.md](https://github.com/gamcoin/gamcoin/blob/master/doc/tor.md) for
details.

## CJDNS-related information in Gamcoin Core

There are several ways to see your CJDNS address in Gamcoin Core:
- in the "Local addresses" output of CLI `-netinfo`
- in the "localaddresses" output of RPC `getnetworkinfo`

To see which CJDNS peers your node is connected to, use `gamcoin-cli -netinfo 4`
or the `getpeerinfo` RPC (i.e. `gamcoin-cli getpeerinfo`).

You can use the `getnodeaddresses` RPC to fetch a number of CJDNS peers known to your node; run `gamcoin-cli help getnodeaddresses` for details.
