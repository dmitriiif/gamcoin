# User-space, Statically Defined Tracing (USDT) for Gamcoin Core

Gamcoin Core includes statically defined tracepoints to allow for more
...
**Note**: Gamcoin Core uses temporary clones of the _main_ UTXO cache
...
## Adding tracepoints to Gamcoin Core
...
Multiple tools can list the available tracepoints in a `gamcoind` binary with
USDT support.

### GDB - GNU Project Debugger

To list probes in Gamcoin Core, use `info probes` in `gdb`:
...
stap net        inbound_message  0x000000000014419e /src/gamcoind
stap net        outbound_message 0x0000000000107c05 /src/gamcoind
stap validation block_connected  0x00000000002fb10c /src/gamcoind
…
```

### With `readelf`

The `readelf` tool can be used to display the USDT tracepoints in Gamcoin Core.
Look for the notes with the description `NT_STAPSDT`.

```
$ readelf -n ./src/gamcoind | grep NT_STAPSDT -A 4 -B 2
