...
Below are some notes on how to build Gamcoin Core for Windows.

The options known to work for building Gamcoin Core on Windows are:
...
Note that for WSL the Gamcoin Core source path MUST be somewhere in the default mount file system, for
example /usr/src/gamcoin, AND not under /mnt/d/. If this is not the case the dependency autoconf scripts will fail.
...
way. This will install to `c:\workspace\gamcoin`, for example:

    make install DESTDIR=/mnt/c/workspace/gamcoin
