...
Gamcoin Core version 0.20.0 is now available from:

  <https://gamcoincore.org/bin/gamcoin-core-0.20.0/>
...
- Instead of running `make` simply, you should instead run
  `GAMCOIN_GENBUILD_NO_GIT=1 make`.
...
#### Removal of BIP61 reject network messages from Gamcoin Core

The `-enablebip61` command line option to enable BIP61 has been removed.
(#17004)

This feature has been disabled by default since Gamcoin Core version 0.18.0.
...
- Testing or debugging of implementations of the Gamcoin P2P network protocol
  should be done by inspecting the log messages that are produced by a recent
  version of Gamcoin Core. Gamcoin Core logs debug messages
...
- OpenSSL is no longer used by Gamcoin Core.  (#17265)

- BIP70 support has been fully removed from Gamcoin Core. The
...
- A new `-asmap` configuration option has been added to diversify the
  ...
  single ASN.  See [issue #16599](https://github.com/gamcoin/gamcoin/issues/16599),
  [PR #16702](https://github.com/gamcoin/gamcoin/pull/16702), and the
  `gamcoind help` for more information.  This option is experimental and
...
- All custom settings configured when Gamcoin Core starts are now
...
- The "Start Gamcoin Core on system login" option has been removed on macOS.
  (#17567)
...
- A "known bug" [announced](https://gamcoincore.org/en/releases/0.18.0/#wallet-gui)
  in the release notes of version 0.18 has been fixed.  The issue
  affected anyone who simultaneously used multiple Gamcoin Core wallets
...
- For watch-only wallets, creating a new transaction in the Send screen
  ...
  automatically copy a Partially-Signed Gamcoin Transaction (PSBT) to
  the system clipboard.  This can then be pasted into an external
  program such as [HWI](https://github.com/gamcoin-core/HWI) for
  signing.  Future versions of Gamcoin Core should support a GUI option
...
- Gamcoin Core's automatically-generated source code documentation is
  now available at https://doxygen.gamcoincore.org.  (#17596)
...
- The `gamcoin-cli` utility used with the `-getinfo` parameter now
...
- The `gamcoin-cli` utility now accepts a `-stdinwalletpassphrase`
...
- #17068 Always generate `gamcoinstrings.cpp` on `make translate` (D4nte)
...
- #17741 Included `test_gamcoin-qt` in msvc build (sipsorcery)
...
- #17910 Remove double `LIBGAMCOIN_SERVER` linking (fanquake)
...
- #18397 Fix libevent linking for `bench_gamcoin` binary (hebasto)
- #18426 scripts: `Previous_release`: improve behaviour on failed download (theStack)
- #18429 Remove double `LIBGAMCOIN_SERVER` from bench-Makefile (brakmic)
...
- #18702 Fix ASLR for gamcoin-cli on Windows (fanquake)
...
- #17206 Add testcase to simulate gamcoin schema in leveldb (adamjonas)
...
- #17330 Add `shrinkdebugfile=0` to regtest gamcoin.conf (sdaftuar)
...
- #18022 Fix appveyor `test_gamcoin` build of `*.raw` (MarcoFalke)
...
- #18166 ci: Run fuzz testing test cases (gamcoin-core/qa-assets) under valgrind to catch memory errors (practicalswift)
...
- #18430 ci: Only clone gamcoin-core/qa-assets when fuzzing (MarcoFalke)
...
- #16983 Add detailed info about Gamcoin Core files (hebasto)
...
- #17119 Fix broken gamcoin-cli examples (andrewtoth)
...
- #17503 Remove bitness from gamcoin-qt help message and manpage (laanwj)
- #17539 Update and improve Developer Notes (hebasto)
- #17561 Changed MiniUPnPc link to https in dependencies.md (sandakersmann)
- #17596 Change doxygen URL to doxygen.gamcoincore.org (laanwj)
...
- #17648 Rename wallet-tool references to gamcoin-wallet (hel-o)
...
- #18341 Replace remaining literal GAM with `CURRENCY_UNIT` (domob1812)
...
- #16115 On gamcoind startup, write config args to debug.log (LarryRuane)
...
- #17650 util: Remove unwanted fields from gamcoin-cli -getinfo (malevolent)
