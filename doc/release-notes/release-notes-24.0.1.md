...
Gamcoin Core version 24.0.1 is now available from:

  <https://gamcoincore.org/bin/gamcoin-core-24.0.1/>
...
This version of Gamcoin Core adds a new `mempoolfullrbf` configuration
...
Some Gamcoin services today expect that the first version of an
...
The Gamcoin Protocol does not, and cannot, provide any assurance that
...
fees, was realized in [Gamcoin Core 0.12.0][] (February 2016) with the
...
default for years, and several contributors to Gamcoin Core are
...
[Gamcoin Core 0.12.0]: https://gamcoincore.org/en/releases/0.12.0/#opt-in-replace-by-fee-transactions
[bip125]: https://github.com/gamcoin/bips/blob/master/bip-0125.mediawiki
...
- The `wsh()` output descriptor was extended with Miniscript support. You can import Miniscript
  ...
  the Gamcoin Core wallet yet.
  You can find more about Miniscript on the [reference website](https://gamcoin.sipa.be/miniscript/). (#24148)

- The `tr()` output descriptor now supports multisig scripts through the `multi_a()` and
  `sortedmulti_a()` functions. (#24043)

- To help prevent fingerprinting transactions created by the Gamcoin Core wallet, change output
...
[documentation](https://github.com/gamcoin/gamcoin/blob/master/doc/managing-wallets.md#migrating-legacy-wallets-to-descriptor-wallets).
...
- Configuration changes made in the gamcoin GUI (such as the pruning setting,
proxy settings, UPNP preferences) are now saved to `<datadir>/settings.json`
file rather than to the Qt settings backend (windows registry or unix desktop
config files), so these settings will now apply to gamcoind, instead of being
ignored. (#15936, gui#602)

- Also, the interaction between GUI settings and `gamcoin.conf` settings is
simplified. Settings from `gamcoin.conf` are now displayed normally in the GUI
settings dialog, instead of in a separate warning message ("Options set in this
dialog are overridden by the configuration file: -setting=value"). And these
settings can now be edited because `settings.json` values take precedence over
`gamcoin.conf` values. (#15936)
